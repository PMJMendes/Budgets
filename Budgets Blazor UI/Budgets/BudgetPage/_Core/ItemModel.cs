...
public class ItemModel : IItem
{
	...
	public void WriteToCSVStream(StreamWriter sw)
	{
		...
		Values.ForEach(v => sw.Write(v.Def.Type switch
		{
			ValueType.NUMBER => v.ProdValue?.ToString() ?? "",
			_ => "\"" + v.TextValue + "\""
		} + ";"));

		for (int i = 0; i < 9 - Values.Count; i++)
		{
			sw.Write(';');
		}

		sw.WriteLine("" + ProdValue + ";");

		Costs.ForEach(c => c.WriteToCSVStream(sw));
	}
	...
	public ItemArgs AsUpdateArgs() => new(
		...
		Costs.Select(c => c.AsUpdateArgs()).ToList()
	);
